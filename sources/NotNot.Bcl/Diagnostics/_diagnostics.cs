...
namespace NotNot.Bcl.Diagnostics
{
	...
	public static class __CHECKED
	{
		...
		[Conditional("CHECKED")]
		[DebuggerNonUserCode, DebuggerHidden]
		public static void WriteLine(string message = null, [CallerArgumentExpression("condition")] string? conditionName = null, [CallerMemberName] string memberName = "", [CallerFilePath] string sourceFilePath = "", [CallerLineNumber] int sourceLineNumber = 0)
		{
	...
	namespace _internal
	{
		...
		public static class DiagHelper
		{
			...
			private static void DoWrite(string eventName, string message, string? conditionName = null, string memberName = "", string sourceFilePath = "", int sourceLineNumber = 0)
			{
				...
				var callsiteFormat = $"{sourceFilePath._GetAfter('\\', true)}:{sourceLineNumber}({memberName})".Color(ConsoleColor.Magenta).Background(ConsoleColor.Black).Bold();
	...
	public struct GcTimingDetails
	{
		...
		public override string ToString()
		{
			if (cachedString == null)
			{
				...
				var pauses = $"{(infoEphemeral.PauseDurations._Sum() + infoBackground.PauseDurations._Sum() + infoFullBlocking.PauseDurations._Sum()).TotalMilliseconds:00.0}ms(EP={infoEphemeral.PauseDurations._Sum().TotalMilliseconds:00}/BG={infoBackground.PauseDurations._Sum().TotalMilliseconds:00}/FB={infoFullBlocking.PauseDurations._Sum().TotalMilliseconds:00})";
...
public class PerfSpikeWatch
{
	...
	public PerfSpikeWatch(string? name = null, double consoleWriteSensitivityFactor = 2.0, double consoleWriteThreshholdMs = 1.0, int pollSkipFrequency = 100)
	{
		if (name == null)
		{
			name = "";
	...
	//[Conditional("CHECKED")]
	public void Lap([CallerMemberName] string memberName = "", [CallerFilePath] string sourceFilePath = "", [CallerLineNumber] int sourceLineNumber = 0)
	{
		...
		if (sampler.IsFilled && (_lapCount % pollSkipFrequency == 0))
		{
			...
			if ((percentiles.p100 >= percentiles.p50 * this.consoleWriteSensitivityFactor && percentiles.p100 > _lastPollPercentiles.p100 * this.consoleWriteSensitivityFactor)
				 ...
				 )
			{
				...
				__ERROR.WriteLine(message, conditionName: "PERFSPIKEWATCH Lap", memberName: memberName, sourceFilePath: sourceFilePath, sourceLineNumber: sourceLineNumber);
			}
			_lastPollPercentiles = percentiles;
		}
	}
	...
	/// <param name="sourceFilePath"></param>
	/// <param name="sourceLineNumber"></param>
	public void LapAndReset([CallerMemberName] string memberName = "", [CallerFilePath] string sourceFilePath = "", [CallerLineNumber] int sourceLineNumber = 0)
	{
